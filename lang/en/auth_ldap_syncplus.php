...
$string['ignore_other_scopes'] = 'Ignore users outside the configured username scope';
$string['ignore_other_scopes_desc'] = 'If set to Yes, users with matching auth type whose username does not end with the configured scope will be ignored during synchronization for user removal checks (suspend/delete) and for user profile updates.';
...
$string['sync_filter_nouserattribute'] = 'Warning: A returned LDAP entry does not have a user attribute set. It must be skipped. Please double check your sync_filter setting to make sure that it does not return entries without user attributes set';
$string['sync_scope'] = 'Username scope when synchronizing users';
$string['sync_scope_desc'] = 'If you are using LDAP Sync Plus to sync users with another authentication type than LDAP, you can configure an additional username scope here. This can become necessary if the user attribute in LDAP does not fully match the username in moodle, especially if the Moodle username is the eduPersonPrincipalName and the username in LDAP is just the uid. The scope is then appended to the LDAP username when syncing the users. Users outside the configured scope are only ignored for removal checks and user profile updates when "Ignore users outside the configured username scope" is set to Yes. A typical example for a scope would be "@example.org".';
...
$string['syncfailedcli'] = 'The user synchronisation has been aborted. Please check the output above for details.';
