...
$string['custom_filter'] = 'Optional: Custom LDAP filter; <strong>Warning:</strong> Do not enter anything when you have no idea what you are doing...<br /><tt>%u</tt> is substituted by the submitted username (PARAM_ALPHANUM) (<tt>%e</tt> for the email-address (PARAM_EMAIL)) in the login-form. Examples: <tt>(&(|(objectclass=person)(objectclass=user))(|(mail=%e)(userPrincipalName=%e)(userPrincipalName=%u@example.com)(otherIpPhone=%e)))</tt><br />Or for a nested group lookup:<tt>(&(&(|(objectclass=person)(objectclass=user))(|(memberof:1.2.840.113556.1.4.1941:=CN=group_with_subgroups,OU=groups,DC=example,DC=com)))(|(userPrincipalName=%e)(mail=%e)(userPrincipalName=%u@stud.thga.de)))</tt>';
